...
from toxbuster.types import Input, Output, Record


class ToxBuster:
    """
    Class for end-to-end inference of Toxbuster model.
    """
    ...
    def __init__(
        ...
        self.taxonomy = convert_id2str_to_taxonomy(self.model.config.id2label)
        self.collator = Collator(
            tokenizer=self.tokenizer,
            label2id={v["label"]: k for k, v in self.taxonomy.items()},
    ...
    def __call__(self, inputs: list[Input]) -> list[Output]:
        ...
        return [postprocess_predictions(r, self.taxonomy, self.version, self.label2threshold) for r in records]
