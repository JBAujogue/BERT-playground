...
class EvaluationMetric:
    ...
    def __call__(self, p) -> dict[str, int | float]:
        ...
        pred_labels = [
            [self.id2label[p] for (p, t) in zip(preds, trues) if t != -100] for preds, trues in zip(pred_ids, true_ids)
        ]
        true_labels = [
            [self.id2label[t] for (p, t) in zip(preds, trues) if t != -100] for preds, trues in zip(pred_ids, true_ids)
        ]
        ...
        results_micro = {"{} {}".format(k, m): results[k][m] for k in keys_micro for m in ["precision", "recall", "f1"]}
        results_macro = {
