...
class OnnxPipelineForFeatureExtraction(Pipeline):
    ...
    def _sanitize_parameters(self, **kwargs):
        ...
        postprocess_kwargs = {"normalize_embeddings": normalize_embeddings} if normalize_embeddings else {}
        return {}, {}, postprocess_kwargs

    def preprocess(self, inputs: str | list[str], **kwargs) -> dict[str, torch.Tensor]:
    ...
    def _mean_pooling(token_embeddings: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        return torch.sum(token_embeddings * masks, 1) / torch.clamp(masks.sum(1), min=1e-9)

    def postprocess(self, model_outputs: dict[str, torch.Tensor], *args, **kwargs) -> list[float | list[float]]:
        pooling = self._mean_pooling(model_outputs["token_embeddings"], model_outputs["attention_mask"])
